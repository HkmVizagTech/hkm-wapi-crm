    ...
    for (const model of models) {
      try {
        ...
        if (!r.ok) {
          ...
          // Auth / quota issues — do NOT retry (avoids hammering an exhausted API)
          if ([401,403].includes(r.status)) return null;
          if (r.status === 429 || err === "RESOURCE_EXHAUSTED" || /quota/i.test(lastErr)) return null;
          continue;
        }
        return d;
      } catch(e) {
...
// Static answers for the most common queries when the Gemini API is down / quota exceeded.
function offlineFallback(message) {
  ...
  if (/donat|seva|gita|annadana|sponsor|offer|contribute|give|pay|link|upi|gpay|phone\s*pe|d[eé]p/.test(m)) {
    return {
      reply: "Hare Krishna 🙏 Thank you for your generosity! You can donate here: https://harekrishnavizag.org/donate\n\nWe've noted your interest — our team is also available at +91 90631 72108.",
...
3. If they want to donate or do ANY seva (Annadana, Gita Daan, monthly, festival, or any name you don't recognize) → give this link directly: https://harekrishnavizag.org/donate AND append ACTION:register_interest. Donation intent NEVER escalates.
...
7. If they need a human (complaints, refunds, complex personal changes) → append ACTION:escalate_to_human
8. Never invent donation amounts, receipt numbers, or promises
9. If you're unsure about anything non-donation, answer with what you know from your training — don't escalate.
