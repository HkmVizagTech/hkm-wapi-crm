...
export function normalizePhone(phone, defaultCountryCode="91") {
  ...
  // Indian numbers: if 10 digits starting with 6-9, add 91
  if (/^[6-9]\d{9}$/.test(p)) p = defaultCountryCode + p;
  // If starts with 0 and 10 digits after, remove 0 and add 91
  if (/^0[6-9]\d{9}$/.test(p)) p = defaultCountryCode + p.slice(1);
  return p;
}
